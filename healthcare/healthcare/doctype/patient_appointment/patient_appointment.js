...
frappe.ui.form.on("Patient Appointment", {
	...
	get_orders: function (frm) {
		...
		const d = new frappe.ui.Dialog({
			...
			primary_action() {
				const selected = d.$wrapper.find(
					'.service-row[data-selected="true"]',
				)[0];
				if (!selected) {
...
let show_procedure_templates = function (frm, result) {
	...
	$.each(result, function (x, y) {
		let row = $(
			repl(
				'<div class="col-xs-12" style="padding-top:12px; text-align:center; display:flex;" >\
